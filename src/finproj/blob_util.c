...
int node_qualifies(image_u32_t* im, node_t* n, metrics_t met)
{
    if(n->num_children < met.min_size) return(0);
    if(met.std_dev_from_square == 0) return(1);
                                    ...
                                    zarray_t* blobs_out)
{
    ...
    for(int i = frame.xy0.y; i < frame.xy1.y; i++) {
        for(int j = frame.xy0.x; j < frame.xy1.x; j++) {
            if((i < frame.ex0.y || i > frame.ex1.y) || (j < frame.ex0.x || j > frame.ex1.x)) {
                ...
                hsv_t hsv = {0,0,0};
                rgb_to_hsv(abgr, &hsv);
                hsv_t error = {fabs(hsv.hue - met.hsv_data[TARGETCOLOR].hsv.hue), 
                                fabs(hsv.sat - met.hsv_data[TARGETCOLOR].hsv.sat), 
                                fabs(hsv.val - met.hsv_data[TARGETCOLOR].hsv.val)};

                // 'Acceptable'
                 if((error.hue < met.hsv_data[TARGETCOLOR].error.hue) && 
                    (error.sat < met.hsv_data[TARGETCOLOR].error.sat) && 
                    (error.val < met.hsv_data[TARGETCOLOR].error.val)) 
                 {
    ...
    */
    ...
    for(int i = 0; i < zarray_size(vals); i++) {
        node_t* node;
        zarray_get(vals, i, &node);
        if(node_qualifies(im, node, met) == 1) {
    ...
    for(int i = 0; i < zarray_size(blobs); i++) {
        ...
        if(buf != NULL) {
            ...
            for(int j = 0; j < 4; j++) {
                vx_buffer_add_back(buf,
                      vxo_pix_coords(VX_ORIGIN_BOTTOM_LEFT,
                             vxo_chain(vxo_mat_translate3(corners[j].x, corners[j].y, 0),
                                 vxo_mat_scale(2.0),
                                 vxo_circle(vxo_mesh_style(vx_maroon)))));   
            }
        }
